...
funHDDCwavelet = function(X,
                          K,
                          minD = 1,
                          maxD = 1,
                          max.level = round(log2(ncol(X))),
                          dimTest = "scree",
                          wavelet.family = "DaubExPhase",
                          wavelet.filter.number = 4,
                          init = "kmeans",
                          minPerClass = 10,
                          threshold = 0.01,
                          minIter = 10,
    ...
    for (K_current in as.vector(K)) {
      for (maxD_current in as.vector(maxD)) {
        for (wavelet.filter.number_current in as.vector(wavelet.filter.number)) {
          result = internal.funHDDCwavelet(
            X,
            K = K_current,
            minD = minD,
            maxD = maxD_current,
            ...
            wavelet.filter.number = wavelet.filter.number_current,
            init = init,
          ...
          result$wavelet.filter.number <-
            wavelet.filter.number_current
...
internal.funHDDCwavelet = function(X,
                                   K,
                                   minD = 1,
                                   maxD = 1,
                                   max.level = round(log2(ncol(X))),
  ...
  X = t(apply(X, 1, function(line) {
    return(
      toWavelet(line, wavelet.family, wavelet.filter.number, max.level = realMaxLevel)
    )
  }))
  ...
  initialClusters <-
    getBalancedCluster(initialClusters, K, minPerClass)
  ...
  for (iter in 1:maxIter) {
    ...
    nbParam <-
      nbParam + K * ncol(X) + K - 1 # number of parameters for mean and pi estimation

    for (l in 1:ncol(sigma$D)) {
      # for each level
